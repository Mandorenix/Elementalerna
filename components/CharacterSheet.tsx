          ...
          {Object.values(ATTRIBUTE_DEFINITIONS).map((attr) => (
            <div key={attr.key} className="flex items-center justify-between bg-gray-800/50 p-2 pixelated-border">
              <div className="flex items-center">
                ...
                <span className="text-xl font-bold text-yellow-300">{character.attributes[attr.key]}</span>
                <span className="text-sm text-gray-400 ml-2">({attr.description})</span>
              </div>
              <div className="flex space-x-2">
                <button
                  onClick={() => onDecreaseAttribute(attr.key)}
                  disabled={character.attributes[attr.key] === character.baseAttributes[attr.key]}
