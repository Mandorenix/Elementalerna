...
const CharacterSheet: React.FC<CharacterSheetProps> = ({
  ...
  return (
    <div className<think>The user said "OK." This means they acknowledge the previous changes.
The previous action was to fix an "Unterminated string constant" error in `constants.tsx` and related type errors in `CharacterSheet.tsx`.
I corrected the string literal for `petrifying_gaze` in `constants.tsx` and ensured `ATTRIBUTE_DEFINITIONS` is correctly exported.
I also updated `CharacterSheet.tsx` to correctly import `ATTRIBUTE_DEFINITIONS` and explicitly type the `attr` variable in the `map` function.

Now, I need to continue improving the UX as per the initial request.
The last UX improvement was for the `EventView.tsx` component, specifically focusing on the combat log and damage popups. I made the combat log more prominent and readable.

Next, I should continue with the `EventView.tsx` component to address the remaining UX points:
1.  **Tydligare skadepopups:** Ensure that damage and healing popups are clearly visible and provide immediate visual feedback. (Partially addressed by making log more prominent, but popups themselves can be improved).
2.  **Statusikoner:** Review how status icons are displayed on characters and enemies to make them more intuitive.

Let's focus on the **status icons** next. They are currently positioned around the sprite, but we can make them more visually distinct and perhaps add a tooltip on hover for better clarity.

I will modify the `ActorSprite` component within `EventView.tsx` to improve the display of status effects.

**Plan:**
1.  Review the current `ActorSprite` component in `EventView.tsx`.
2.  Enhance the visual presentation of status effect icons.
3.  Ensure tooltips are available for each status effect to explain what it does. (This might require a new `StatusEffectTooltip` component or integrating existing `Tooltip` logic).

Let's start by making the status icons more organized and potentially adding a simple hover effect. The current implementation already maps icons and durations, but we can refine their positioning and add a background for better contrast.

I will modify `components/EventView.tsx` to adjust the styling and positioning of status effect icons on actors. I'll also ensure the `title` attribute is used for basic tooltip functionality.

**Detailed changes for `components/EventView.tsx`:**
- Adjust the `status-icon` styling to ensure better visibility and positioning.
- Ensure the `title` attribute on the `status-icon` div provides a clear description of the effect and its duration.
- I will also ensure that the `statusIconMap` in `EventView.tsx` has comprehensive titles for all status effects.

Let's refine the `ActorSprite` component within `EventView.tsx`.Absolut! Vi fortsätter att förbättra användarupplevelsen i stridsvyn. Efter att ha justerat stridsloggen, kommer jag nu att fokusera på **statusikonerna** för att göra dem tydligare och mer intuitiva.

Jag kommer att modifiera `ActorSprite`-komponenten i `components/EventView.tsx` för att förbättra hur statusikoner visas på karaktärer och fiender. Detta inkluderar att justera deras positionering, lägga till en bakgrund för bättre kontrast, och se till att de har tydliga `title`-attribut för grundläggande verktygstips vid hovring.

Här är ändringarna i `components/EventView.tsx`:

<dyad-write path="components/EventView.tsx" description="Förbättrar visningen av statusikoner på aktörer i stridsvyn för ökad tydlighet och användarvänlighet.">
...
const EventView: React.FC<{
  ...
  const performAttack = useCallback(async (attackerId: string, defenderId: string, baseDamage: number, damageElement: Element = Element.NEUTRAL, isSkill: boolean = false, skillEffect?: () => void) => {
      ...
      let hettaBonus = 0;
      let hettaConsumed = 0; // New variable for Hetta consumption
      if (attacker.type === 'PLAYER' && character.archetype === 'Pyromanten') {
        hettaBonus = Math.floor(playerResource / 10); // Calculate bonus based on current Hetta
        if (hettaBonus > 0) {
            hettaConsumed = hettaBonus * 10; // Consume Hetta that contributed to the bonus
            setPlayerResource(r => Math.max(0, r - hettaConsumed)); // Update player resource
        }
      }
      ...
      if(isDodge) {
          ...
          const hettaLog = hettaBonus > 0 ? ` (+${hettaBonus} från Hetta, -${hettaConsumed} Hetta)` : ""; // Update log message
          ...
          if (attacker.type === 'PLAYER') {
            // FIX: Explicitly type the array from Object.values to prevent 'unknown' type errors.
  ...
  const handlePlayerAction = async (action: 'ATTACK' | 'SKILL' | 'DEFEND' | 'ULTIMATE', abilityId?: string) => {
      ...
      switch (character.archetype) {
        case 'Pyromanten':
            // Pyromancer skills generate Hetta, so they don't have a cost to check for affordability.
            // The 'effectiveCost' for Pyromancer is 0 for the canAfford check.
            // Hetta generation and overheating is handled below.
            if (player.statusEffects.some(e => e.type === 'overheated')) {
                addLogMessage("Du är överhettad och kan inte generera Hetta!");
                // No Hetta gain if overheated
            } else {
                const hettaGained = rankData ? rankData.resourceCost : 0; // This is the Hetta generated by the skill
                ...
                if (newHetta >= playerStats.maxAether) {
                    ...
                    setPlayerResource(0); // Reset Hetta after explosion
                } else {
            ...
            case 'mud': // Lerfälla
             ...
             case 'sand': // Sandstorm
              ...
              for (const enemy of livingEnemies) {
                  ...
                  updateActorState(enemy.id, { hp: newHp, statusEffects: enemy.statusEffects, animationState: 'hit' });
  ...
  const ActorSprite: React.FC<{actor: Actor; onDefeatAnimationEnd: (id: string) => void; equipment?: Record<EquipmentSlot, Item | null>}> = ({ actor, onDefeatAnimationEnd, equipment }) => {
    ...
    const statusIconMap: Partial<Record<StatusEffect['type'], { icon: React.FC; title: string; }>> = {
        'defending': { icon: Icons.Shield, title: "Försvarar" },
        'hasted': { icon: Icons.Wind, title: "Haste" },
        'burning': { icon: Icons.Burn, title: "Brinnande" },
        'poisoned': { icon: Icons.Poison, title: "Förgiftad" },
        'slowed': { icon: Icons.Slow, title: "Förlångsammad" },
        'blinded': { icon: Icons.Blinded, title: "Bländad" },
        'retaliating': { icon: Icons.Magma, title: "Vedergällande" },
        'full_flow': { icon: Icons.FullFlow, title: "Fullt Flöde" },
        'overheated': { icon: Icons.Overheat, title: "Överhettad" },
        'rooted': { icon: Icons.Rooted, title: "Rotad" },
        'steamed': { icon: Icons.Steamed, title: "Ångad" },
        'regenerating': { icon: Icons.Regenerating, title: "Regenererar" },
        'frozen': { icon: Icons.Frozen, title: "Frusen" },
        'paralyzed': { icon: Icons.Paralyzed, title: "Förlamad" },
        'petrified': { icon: Icons.Petrified, title: "Förstenad" },
        'frail': { icon: Icons.Frail, title: "Förgänglig" },
        'frightened': { icon: Icons.Frightened, title: "Förskräckt" },
    };
        ...
        >
          ...
          <div className="relative transform scale-[3] w-12 h-12 flex items-center justify-center">
            ...
            <div className="status-vfx-overlay">
                ...
                {actor.statusEffects.some(e => e.type === 'steamed') && <div className="vfx-steamed-overlay" />}
                {actor.statusEffects.some(e => e.type === 'petrified') && <div className="vfx-petrified-overlay" />}
                {actor.statusEffects.some(e => e.type === 'frail') && <div className="vfx-frail-overlay" />}
                {actor.statusEffects.some(e => e.type === 'frightened') && <div className="vfx-frightened-overlay" />}
            </div>
            {/* Status Effect Icons */}
                ...
                {actor.statusEffects.map((effect, index) => {
                    ...
                    return (
                        <div
                            ...
                            title={`${iconData.title} (${effect.duration} rundor kvar)`}
                        >
                            <Icon />
  ...
  return (
    <div className="flex-grow w-full h-full p-2 text-white flex flex-col items-center justify-center bg-black/80 relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-48 flex z-20">
            ...
            {showCommandMenu && (
                <div className="ff-panel w-48">
                    <ul>
                        ...
                        <li onClick={() => { setShowCommandMenu(false); setShowUltimateMenu(true); }} className="p-1 hover:bg-white/20 cursor-pointer">Ultimata</li>
            ...
            {/* Log */}
            <div className="ff-panel flex-grow h-full overflow-hidden p-4">
                 {log.map((msg, i) => <p key={i} className="text-sm leading-tight mb-1">{msg}</p>)}
            </div>
